...
//Modified
    ...
    function driveproxy($id) {
        ...
        foreach($data as $d) {
            ...
            $o[$r] = substr(preg_replace(array("@&driveid=(.+?)&@si","/\/[^\/]+\.google\.com\/videoplayback/","@&ip=(.+?)&@si"),array("&driveid=$hashdrive&api=japnime&","/json.japnimeserver.com/drive.php","&ip=$1&ck=$gck[0]&dom=$hiddomain&"), $d), 3);
